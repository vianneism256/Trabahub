...
const [notifications, setNotifications] = useState([])
  ...
  async function loadCustomerProfile() {
    try {
      ...
      const bal = await connectService.getBalance(currentUser.uid, 'customer')
      setConnects(bal)
    } catch (err) {
      console.error(err)
    }
  ...
  async function handlePostJob(e) {
    ...
    try {
      ...
      if (connects !== null && connects < connectService.COSTS.postJob) {
        setMessage(`Not enough connects. Posting a job costs ${connectService.COSTS.postJob} connects. You have ${connects}. Buy more in the Buy Connects tab.`)
        setLoading(false)
        return
      }
      if (connects !== null) {
        const newBal = await connectService.deductConnects(currentUser.uid, 'customer', connectService.COSTS.postJob)
        setConnects(newBal)
      }
...
async function handleAccept(conversationId) {
  ...
  async function handleSubmitPurchase() {
    ...
    try {
      await connectService.submitPurchase(currentUser.uid, 'customer', selectedPackage.connects, selectedPackage.price, receiptFile)
  ...
  async function handleMarkNotifAsRead(notifId) {
    try { await notificationService.markAsRead(notifId) } catch (err) { console.error(err) }
  }

  async function handleMarkAllNotifsAsRead() {
    try { await notificationService.markAllAsRead(currentUser.uid) } catch (err) { console.error(err) }
  }
          ...
          <div style={{ maxWidth: 700 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              ...
              {notifications.some((n) => !n.isRead) && (
                <button onClick={handleMarkAllNotifsAsRead} style={{ padding: '8px 16px', backgroundColor: 'var(--primary)', color: 'white', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>
                  Mark all as read
                </button>
              )}
            </div>
            {notifications.length === 0 ? (
              <div style={{ backgroundColor: 'white', padding: 60, borderRadius: 8, textAlign: 'center', color: 'var(--gray-600)', boxShadow: 'var(--shadow-sm)' }}>
                <p style={{ fontSize: 40, margin: '0 0 12px 0' }}>🔔</p>
                <p style={{ fontSize: 18, fontWeight: 600, marginBottom: 8 }}>No notifications yet</p>
                <p style={{ fontSize: 14, margin: 0 }}>We'll notify you about connect purchases and job updates</p>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {notifications.map((notif) => (
                  <div
                    key={notif.id}
                    onClick={() => !notif.isRead && handleMarkNotifAsRead(notif.id)}
                    style={{
                  ...
                  >
                    <div style={{ fontSize: 22, flexShrink: 0, marginTop: 2 }}>
                      {notif.type === 'connects_approved' && '💰'}
                      {notif.type === 'connects_rejected' && '❌'}
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                        <p style={{ margin: 0, fontWeight: 700, fontSize: 14, color: 'var(--gray-900)' }}>{notif.title}</p>
                        <span style={{ fontSize: 12, color: 'var(--gray-500)', whiteSpace: 'nowrap', marginLeft: 12 }}>{new Date(notif.createdAt).toLocaleDateString()}</span>
                      </div>
                      <p style={{ margin: 0, fontSize: 13, color: 'var(--gray-700)', lineHeight: 1.5 }}>{notif.message}</p>
                    </div>
                    {!notif.isRead && <div style={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: 'var(--primary)', marginTop: 6, flexShrink: 0 }} />}
                  </div>
                ))}
              </div>
            )}
          ...
          <div style={{ maxWidth: 760 }}>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <p style={{ margin: '0 0 20px 0', fontSize: 14, color: 'var(--gray-600)' }}>
                Posting a job costs <strong>4 connects</strong>. Connects never expire.
              </p>
            ...
            {selectedPackage && (
              <div style={{ backgroundColor: 'white', padding: 28, borderRadius: 8, boxShadow: 'var(--shadow-sm)', border: '1.5px solid var(--primary-light)' }}>
                ...
                <div style={{ borderTop: '1px solid var(--gray-200)', paddingTop: 20 }}>
                  ...
                  <p style={{ fontSize: 13, color: 'var(--gray-600)', margin: '0 0 16px 0' }}>After sending ₱{selectedPackage.price} via GCash, take a screenshot of the confirmation and upload it here.</p>
                  <input ref={receiptFileRef} type="file" accept="image/*" onChange={(e) => setReceiptFile(e.target.files?.[0] || null)} style={{ display: 'none' }} />
                  {receiptFile ? (
                    ...
                    <button onClick={() => receiptFileRef.current?.click()} style={{ padding: '12px 24px', backgroundColor: 'white', color: 'var(--gray-700)', border: '1.5px dashed var(--gray-300)', borderRadius: 8, cursor: 'pointer', fontWeight: 600, fontSize: 14, marginBottom: 16 }}>
                  ...
                  <div style={{ display: 'flex', gap: 12 }}>
                    <button
                      ...
                      style={{ flex: 1, padding: '14px 24px', backgroundColor: receiptFile ? 'var(--primary)' : 'var(--gray-300)', color: 'white', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: 15, cursor: receiptFile ? 'pointer' : 'not-allowed' }}
                    >
                      {submittingPurchase ? 'Submitting...' : `Submit Receipt — ${selectedPackage.connects} Connects for ₱${selectedPackage.price}`}
                    </button>
                    <button onClick={() => { setSelectedPackage(null); setReceiptFile(null); if (receiptFileRef.current) receiptFileRef.current.value = '' }} style={{ padding: '14px 20px', backgroundColor: 'var(--gray-100)', color: 'var(--gray-700)', border: 'none', borderRadius: 8, fontWeight: 600, cursor: 'pointer', fontSize: 14 }}>
                      Cancel
                    </button>
                  </div>
                </div>
